...
export const ProductosTable = ({ data, onEdit, onDelete }) => (
  <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
    {data.length === 0 ? (
      <div className="text-center py-16 px-4">
        ...
        <p className="text-gray-600 font-medium">No hay productos registrados</p>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase">Dimensiones</th>
              <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase">Vida Útil</th>
              ...
              <tr key={item.id} className="hover:bg-blue-50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
                    item.tipoProducto === 'COMESTIBLE' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                  }`}>
                    {item.tipoProducto}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-sm text-gray-600">
                    {item.largo}×{item.ancho} (Vol: {item.volumen})
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-sm text-gray-600">{item.tiempoVidaUtilBase} días</span>
                </td>
